...
These final characters were chosen to avoid conflicts with existing standardized
DCS sequences.
Sixel uses \code{DCS} with a numeric parameter and final byte \code{q};
DECRQSS uses intermediate byte \code{\$} with final byte \code{q}.
Both are distinguishable because \GoodImageProtocol sequences carry no
intermediate bytes and no numeric parameters before the final byte.
...
\begin{tabular}{|m{3cm}|m{2cm}|m{11cm}|}
    ...
    \textbf{f}             & format           & 1 (RGB), 2 (RGBA), 3 (PNG). Required. \\
...
\begin{itemize}
    ...
    \item A version negotiation mechanism (e.g.\ extended DA response or query response)
        may be added if protocol extensions require feature detection
        beyond presence/absence of the \DA code~\code{11}.
\end{itemize}
